# ui_components.py (Versão completa com funcionalidade de IA, Anexos e Configuração de Notificações)
...
from config_utils import save_config # <-- Adicionado para a nova view de notificação
...
async def _build_notion_page_content(config: dict, thread_context: Optional[discord.Thread], notion_integration: NotionIntegration) -> Optional[List[Dict]]:
        ...
        if messages:
            ...
            if summary_text and not summary_text.startswith("Erro:"):
                page_content.append({
                    ...
                    "heading_2": {"rich_text": [{"type": "text", "text": {"content": "🤖 Resumo da IA"}}]}
                })
    ...
    if attachments:
        ...
        page_content.append({
            "object": "block", "type": "heading_2",
            "heading_2": {"rich_text": [{"type": "text", "text": {"content": "📎 Anexos do Tópico"}}]}
        })
        for att in attachments:
            if att['type'] == 'image':
                ...
                page_content.append({
                    ...
                    "paragraph": {"rich_text": [
                        {"type": "text", "text": {"content": f"Vídeo/GIF ({att['filename']}): "}},
                        {"type": "text", "text": {"content": att['url'], "link": {"url": att['url']}}}
                    ]}
                })
...
# --- CLASSE MODIFICADA E ADICIONADA ---

class NotificationConfigView(View):
    """View para configurar as opções de notificação do Notion para o Discord."""
    def __init__(self, guild_id: int, channel_id: int, config: dict):
        ...
        # Atualiza a aparência do botão com base no estado atual da configuração
        is_enabled = self.config.get('topic_notifications_enabled', False)
        self.toggle_topic_button.label = "Desativar Notificações em Tópicos" if is_enabled else "Ativar Notificações em Tópicos"
        self.toggle_topic_button.style = ButtonStyle.danger if is_enabled else ButtonStyle.success

    @discord.ui.button(label="Ativar/Desativar Notificações em Tópicos", style=ButtonStyle.secondary, row=0)
    async def toggle_topic_button(self, interaction: Interaction, button: Button):
        # Verifica se a propriedade de link do tópico está configurada, pois é um pré-requisito
        if not self.config.get('topic_link_property_name'):
            await interaction.response.edit_message(
                content="❌ **Erro:** Primeiro configure a 'Propriedade de Link do Tópico' no menu de gerenciamento anterior.",
                view=None
            )
            return

        # Inverte o estado atual (se não existir, considera False e o torna True)
        new_state = not self.config.get('topic_notifications_enabled', False)
        save_config(self.guild_id, self.channel_id, {'topic_notifications_enabled': new_state})
        
        status_text = "ATIVADAS" if new_state else "DESATIVADAS"
        await interaction.response.edit_message(content=f"✅ Notificações em tópicos foram **{status_text}**.", view=None)

    # TODO: Adicionar botões aqui para configurar "Canal Fixo" e "DM" no futuro.
...
class ManagementView(View):
    def __init__(self, parent_interaction: Interaction, notion: NotionIntegration, config: dict):
        ...
        # CORRIGIDO: Garante que pegamos o ID do canal, não do tópico, para carregar a config
    ...
    async def configure_notifications(self, interaction: Interaction, button: Button):
        """Abre a view para configurar as notificações do Notion para o Discord."""
        await interaction.response.send_message(
            "Selecione o tipo de notificação que deseja configurar:",
            view=NotificationConfigView(self.guild_id, self.channel_id, self.config),
            ephemeral=True
        )
        self.stop()