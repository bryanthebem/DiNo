...
class NotionIntegration:
    ...
    def __init__(self):
        ...
        if not self.token:
            raise ValueError(
                "O token do Notion (NOTION_TOKEN) não foi encontrado no seu ambiente."
            )
    ...
    def _format_property_value(self, prop_type: str, prop_value):
        ...
        if prop_type == 'title':
            return {"title": [{"text": {"content": str(prop_value)}}]}
        elif prop_type == 'rich_text':
            return {"rich_text": [{"text": {"content": str(prop_value)}}]}
        elif prop_type == 'url':
            return {"url": prop_value}
        elif prop_type == 'status':
            return {"status": {"name": str(prop_value)}}
        elif prop_type == 'select':
            value = prop_value[0] if isinstance(prop_value,
                                                list) else prop_value
            return {"select": {"name": str(value)}}
        elif prop_type == 'multi_select':
            tags_to_add = prop_value if isinstance(prop_value, list) else [
                tag.strip() for tag in str(prop_value).split(',')
                if tag.strip()
            ]
            ...
            if date_obj:
                ...
                print(
                    f"Aviso: Não foi possível interpretar a data '{prop_value}'."
                )
                return None
            ...
            try:
                ...
                print(f"Aviso: {e}. Propriedade 'people' será ignorada.")
        return None
    ...
    def _convert_text_to_notion_rich_text_objects(self, text_content: str):
        ...
        for part in parts:
            ...
            if part.startswith('**') and part.endswith('**') and len(
                    part) >= 4:
                annotations["bold"] = True
                clean_text = part[2:-2]
            elif part.startswith('_') and part.endswith('_') and len(
                    part) >= 2:
        ...
        for line in lines:
                ...
                notion_blocks.append({
                    ...
                    "paragraph": {
                        "rich_text": [{
                            "type": "text",
                            "text": {
                                "content": bold_heading_match.group(1) + ":"
                            },
                            "annotations": {
                                "bold": True
                            }
            ...
            # Tenta identificar itens de lista como * Item ou - Item
                ...
                notion_blocks.append({
                    ...
                    "bulleted_list_item": {
                        "rich_text":
                        self._convert_text_to_notion_rich_text_objects(
                            content_text)
                    }
                })
            else:
                ...
                notion_blocks.append({
                    ...
                    "paragraph": {
                        "rich_text":
                        self._convert_text_to_notion_rich_text_objects(line)
                    }
                })
    ...
    def search_in_database(self,
        ...
        if property_type in ["rich_text", "title"]:
            ...
            if pessoa_id:
                ...
                return {
                    "results": []
                }  # Se não encontrar a pessoa, retorna uma busca vazia para não dar erro
        try:
    ...
    def get_database_properties(self, url):
        ...
        try:
            ...
            raise NotionAPIError(f"Erro ao obter propriedades do Notion: {e}")
    ...
    def search_id_person(self, search_term: str):
        ...
        try:
            ...
            raise NotionAPIError(
                f"Não foi possível buscar os usuários no Notion.")
    ...
    def get_database_count(self, url):
        ...
        try:
            ...
            raise NotionAPIError(f"Erro ao contar páginas no Notion: {e}")
    ...
    def build_page_properties(self, db_url: str, title: str,
                              properties_dict: dict):
        schema = self.get_database_properties(db_url)
        page_properties = {}
        title_prop_name = next(
            (name for name, data in schema.items() if data['type'] == 'title'),
            None)
        if title_prop_name:
            page_properties[title_prop_name] = self._format_property_value(
                'title', title)
        ...
        for prop_name, prop_value in properties_dict.items():
            ...
            if not prop_data:
                print(
                    f"AVISO: A propriedade '{prop_name}' não foi encontrada na base de dados. Ela será ignorada."
                )
                continue
            formatted_prop = self._format_property_value(
                prop_data.get('type'), prop_value)
            if formatted_prop:
    ...
    def extract_value_from_property(self, prop_data, prop_type):
        try:
            if prop_type == 'title':
                ...
                return "".join([
                    part.get('plain_text', '')
                    for part in prop_data.get('rich_text', [])
                ])
            elif prop_type == 'status':
                ...
                return ", ".join([
                    person.get('name', 'Usuário Desconhecido')
                ...
                if date_info and date_info.get('start'):
                    return datetime.fromisoformat(
                        date_info['start']).strftime('%d/%m/%Y')
                return ''
            elif prop_type == 'url':
    ...
    def get_properties_for_interaction(self, url):
        ...
        excluded_types = [
            'rollup', 'created_by', 'created_time', 'last_edited_by',
            'last_edited_time', 'formula'
        ]
        for prop_name, prop_data in all_props.items():
            ...
            if prop_type == 'select':
                prop_info['options'] = [
                    opt['name']
                    for opt in prop_data.get('select', {}).get('options', [])
                ]
            elif prop_type == 'multi_select':
                prop_info['options'] = [
                    opt['name']
                    for opt in prop_data.get('multi_select', {}).get(
                        'options', [])
                ]
            elif prop_type == 'status':
                prop_info['options'] = [
                    opt['name']
                    for opt in prop_data.get('status', {}).get('options', [])
                ]
    ...
    def update_page(self, page_id: str, properties: dict):
        try:
            return self.notion.pages.update(page_id=page_id,
                                            properties=properties)
        except Exception as e:
            raise NotionAPIError(f"Erro ao atualizar a página no Notion: {e}")
    ...
    def delete_page(self, page_id: str):
        ...
        try:
            ...
            raise NotionAPIError(
                f"Erro ao deletar (arquivar) a página no Notion: {e}")
